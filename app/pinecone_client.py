...
def upsert_vector(vector_id: str, product_id: str, embedding: list, category: str) -> dict:
    if index is None:
    ...
    print(f"✅ Upsert complete: {upserted} vector(s) added (ID: {vector_id})")
...
def query_similar(embedding: list, top_k: int = 20) -> list:
    if index is None:
        raise RuntimeError("Pinecone index not initialized. Call init_index() first.")
    ...
    # Deduplicate by product_id, keep highest score per product, filter by threshold
    seen: dict = {}
